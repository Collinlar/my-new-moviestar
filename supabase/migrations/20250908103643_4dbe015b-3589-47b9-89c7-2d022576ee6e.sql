...
-- Create triggers for helpful votes count
...
-- Create triggers for flag count
...
-- Allow users to view basic public information of other users (display_name, avatar_url, bio)
CREATE POLICY "Public profile info viewable by everyone" 
ON public.profiles 
FOR SELECT 
USING (true);

-- But sensitive information should only be viewable by the profile owner or admins
...
-- Add function to get trending movies (referenced in useMovies.tsx)
